...
public class CONVSTR {
  ...
  private static char min(String s1, ArrayList<Integer> list) {
    char ch = s1.charAt(list.get(0));
    for (int i = 1; i < list.size(); i++) {
      if (s1.charAt(list.get(i)) < ch)
        ch = s1.charAt(list.get(i));
    }
    return ch;
  }
  // Check Prime :
